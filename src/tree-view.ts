...
export namespace BranchesTreeView {
	...
	let foldState = Number(provider.expandBranches) + Number(provider.expandBranches && provider.expandUnmergedDetails);
	...
	export async function initView(): Promise<void> {
		...
		Janitor.add(
			...
			//MO TODO add switch repo
			commands.registerCommand("git-branches.reloadView", () => provider.reloadItems()),
	...
	/**
	 * Toggles view fold status: Layer1, Layer2, Collapsed
	 */
